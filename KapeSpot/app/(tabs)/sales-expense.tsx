...
import ReactNativeBlobUtil from 'react-native-blob-util'; // ADD THIS IMPORT FOR EXPORT
...
export default function SalesExpenseScreen() {
    ...
    // ============== NEW EXPORT FUNCTION FOR EXPENSES (MATCHING LOG.TSX APPROACH) ==============
    
    // Generate CSV content for expenses - EXACT FORMAT AS REQUESTED
    const generateExpensesCSV = (expenses: ExpenseDocument[]): string => {
        ...
        expenses.forEach((expenseDoc, docIndex) => {
            const batchNumber = docIndex + 1;
            const batchDate = expenseDoc.createdAt ? 
                new Date(expenseDoc.createdAt).toLocaleString() : 'Unknown Date';
            
            csvContent += `BATCH ${batchNumber}\n`;
            csvContent += `Date: ${batchDate}\n`;
            csvContent += `----------------------------------------\n`;
            csvContent += `Item,Cost\n`;
            
            expenseDoc.expenses.forEach((expense) => {
                const description = expense.description.replace(/,/g, ' '); // Remove commas to avoid CSV issues
                const cost = `₱${expense.cost.toFixed(2)}`;
                csvContent += `${description},${cost}\n`;
                totalItems++;
            });
            
            csvContent += `----------------------------------------\n`;
            csvContent += `Batch Total:,₱${expenseDoc.total.toFixed(2)}\n\n`;
            grandTotal += expenseDoc.total;
        });
        
        csvContent += `==================================================\n`;
        csvContent += `GRAND TOTAL:,₱${grandTotal.toFixed(2)}\n`;
        csvContent += `Total Batches:,${expenses.length}\n`;
    ...
    // Helper function to download file (same as log.tsx)
        ...
        try {
            ...
            if (Platform.OS === 'android') {
                ...
                await ReactNativeBlobUtil.android.addCompleteDownload({
                    title: fileName,
                    description: 'KapeSpot Expenses Report',
                    mime: 'text/csv',
                    path: filePath,
    ...
    // Main export function for expenses - MATCHING LOG.TSX APPROACH
    const exportExpensesToCSV = async () => {
        ...
        try {
            ...
            const csvContent = generateExpensesCSV(allExpenses);
            const timestamp = new Date().toISOString().split('T')[0];
            const fileName = `KapeSpot_Expenses_Report_${timestamp}.csv`;
            ...
            >
                ...
                <ScrollView style={styles.scrollContainer} showsVerticalScrollIndicator={false}>
                    <ThemedView style={styles.content}>
                            ...
                            <ThemedView style={styles.headerSection}>
                                <ThemedView style={styles.headerTop}>
                                    ...
                                    <ThemedView style={styles.headerButtons}>
                                        ...
                                        >
                                            <Feather name="dollar-sign" size={18} color="#854442" />
                                        </TouchableOpacity>
                                        <TouchableOpacity
                                            style={styles.listButton}
                                            onPress={openExpensesModal}
                                        >
                                            <Feather name="list" size={18} color="#854442" />
                                        </TouchableOpacity>
                                        {/* Printer Button */}
                                        <TouchableOpacity
                                            style={[
                        ...
                        {/* All Expenses View Modal - UPDATED WITH EXPORT BUTTON MATCHING LOG.TSX */}
                        <Modal
                            animationType="slide"
                            transparent={true}
                                ...
                                >
                                    <ThemedView style={styles.modalContent}>
                                        ...
                                        {loadingExpenses ? (
                                            ...
                                            <ThemedView style={styles.noDataContainer}>
                                                <Feather name="file-text" size={32} color="#854442" />
                        ...
                        <ThemedView style={styles.chartsRow}>
                            ...
                            <ThemedView style={styles.chartSection}>
                                ...
                                {salesData.length > 0 ? (
                                    ...
                                    <ThemedView style={styles.noDataContainer}>
                                        <Feather name="bar-chart-2" size={32} color="#854442" />
                        ...
                        <ThemedView style={styles.recentActivitySection}>
                            <ThemedView style={styles.activityHeader}>
                                ...
                                <ThemedText style={styles.activitySubtitle}>Transactions .</ThemedText>
                            </ThemedView>
...
const styles = StyleSheet.create({
    ...
    chartSection: {
        flex: 1,
        backgroundColor: "rgba(223, 204, 175, 1)",
        borderRadius: 12,
        padding: 12,
        marginHorizontal: 6,
        borderWidth: 1,
